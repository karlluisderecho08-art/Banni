...
public class DatabaseService
{
    ...
    {
        using var conn = GetConnection();
        return (await conn.QueryAsync<Pet, Owner, Pet>(@"
            SELECT p.*, o.Id, o.FullName, o.Email, o.Phone
            FROM Pets p 
            JOIN Owners o ON p.OwnerId = o.Id
            ORDER BY p.Name",
            (pet, owner) => {
                pet.Owner = owner;  // 👈 maps the Owner object onto the Pet
                return pet;
            },
            splitOn: "Id"  // 👈 tells Dapper where the Owner columns start
        )).ToList();
    }
